...
import {BundlingOptions, Duration, Stack} from 'aws-cdk-lib';
                        ...
                        bundling: {
                            ...
                            local: {
                                tryBundle(outputDir: string, options: BundlingOptions): boolean {
                                    ...
                                    execSync(`pip3 install -r ${path.join(`${lambdaBaseDir}/accounts`, "requirements.txt")} -t ${path.join(outputDir)}`);
                                    execSync(`cp -a ${lambdaBaseDir}/accounts/* ${path.join(outputDir)}`);
                                    return true;
                                }
                            }
                        }
                        ...
                        bundling: {
                            ...
                            local: {
                                tryBundle(outputDir: string, options: BundlingOptions): boolean {
                                    ...
                                    execSync(`pip3 install -r ${path.join(`${lambdaBaseDir}/scan`, "requirements.txt")} -t ${path.join(outputDir)}`);
                                    execSync(`cp -a ${lambdaBaseDir}/scan/* ${path.join(outputDir)}`);
                                    return true;
                                }
                            }
                        ...
                        bundling: {
                            ...
                            local: {
                                tryBundle(outputDir: string, options: BundlingOptions): boolean {
                                    ...
                                    execSync(`pip3 install -r ${path.join(`${lambdaBaseDir}/takeover`, "requirements.txt")} -t ${path.join(outputDir)}`);
                                    execSync(`cp -a ${lambdaBaseDir}/takeover/* ${path.join(outputDir)}`);
                                    return true;
                                }
                            }
                        }
                        ...
                        bundling: {
                            ...
                            local: {
                                tryBundle(outputDir: string, options: BundlingOptions): boolean {
                                    ...
                                    execSync(`pip3 install -r ${path.join(`${lambdaBaseDir}/stats`, "requirements.txt")} -t ${path.join(outputDir)}`);
                                    execSync(`cp -a ${lambdaBaseDir}/stats/* ${path.join(outputDir)}`);
                                    return true;
                                }
                            }
                        }
                        ...
                        bundling: {
                            ...
                            local: {
                                tryBundle(outputDir: string, options: BundlingOptions): boolean {
                                    ...
                                    execSync(`pip3 install -r ${path.join(`${lambdaBaseDir}/notify`, "requirements.txt")} -t ${path.join(outputDir)}`);
                                    execSync(`cp -a ${lambdaBaseDir}/notify/* ${path.join(outputDir)}`);
                                    return true;
                                }
                            }
                        }
                        ...
                        bundling: {
                            ...
                            local: {
                                tryBundle(outputDir: string, options: BundlingOptions): boolean {
                                    ...
                                    execSync(`pip3 install -r ${path.join(`${lambdaBaseDir}/resources`, "requirements.txt")} -t ${path.join(outputDir)}`);
                                    execSync(`cp -a ${lambdaBaseDir}/resources/* ${path.join(outputDir)}`);
                                    return true;
                                }
                            }
                        }
                        ...
                        bundling: {
                            ...
                            local: {
                                tryBundle(outputDir: string, options: BundlingOptions): boolean {
                                    ...
                                    execSync(`pip3 install -r ${path.join(`${lambdaBaseDir}/update`, "requirements.txt")} -t ${path.join(outputDir)}`);
                                    execSync(`cp -a ${lambdaBaseDir}/update/* ${path.join(outputDir)}`);
                                    return true;
                                }
                            }
                        }
                        ...
                        bundling: {
                            ...
                            local: {
                                tryBundle(outputDir: string, options: BundlingOptions): boolean {
                                    ...
                                    execSync(`pip3 install -r ${path.join(`${lambdaBaseDir}/current`, "requirements.txt")} -t ${path.join(outputDir)}`);
                                    execSync(`cp -a ${lambdaBaseDir}/current/* ${path.join(outputDir)}`);
                                    return true;
                                }
                            }
                        }
